...
<template>
 <CardBox :class="cardClass" is-form @submit.prevent="update_student">
  ...
  <div v-if="success_message == true"
      ...
      <BaseButtons style="margin:5px">
        ...
        <BaseButton :icon="mdiContentSave"  type="submit" color="info" label="Update Student" />
...
export default {
  ...
  return {
    ...
    student_id:this.$router.currentRoute.value.params.id,
    image:'',
    attachment:'',
  }
},
...
mounted() {
  if (!this.user_info) {
    ...
    this.get_student(this.student_id)
...
const response = await axios.get(`http://localhost:8000/Api/get_student/${id}`,
  {
  headers: {
...
let data=response.data.student
          ...
          this.email_student=data.email
          this.password=""
          this.Date_Birth =data.Date_Birth
          this.nationalitie_id=data.nationalitie_id
          this.blood_id =data.blood_id
...
// handle error
    ...
    try {
      ...
      this.get_mother(this.parent_id)
      this.get_classroom(this.grade)
      this.get_section(this.classroom)
      }
  ...
  async update_student() {
    ...
    formData.append("section_id",this.section_id);
        ...
        const response = await axios.post('http://localhost:8000/Api/update_student',formData,
          {
            headers: {
              'auth_token': this.userData.user.api_token,
              'api_password': "ase1iXcLAxanvXLZcgh6tk",
